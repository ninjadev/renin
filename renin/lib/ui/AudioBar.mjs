import { BoxBufferGeometry, Texture, CanvasTexture, Object3D, Mesh, MeshBasicMaterial, ShaderMaterial, RepeatWrapping } from 'three';
import { colors } from './colors.mjs';
import { defaultVertexShader } from '../renin.mjs';
import { UIBox, makeRoundedRectangleBufferGeometry } from './UIBox.mjs';
import { getWindowHeight, gradientCanvas } from '../utils.mjs';
import audioBarShader from './audioBarShader.mjs';
...
class AudioBar {
    ...
    render(renin, cuePoints) {
        ...
        let deepestDepth = 0;
        const recurse = (node, depth = 0, startFrameBound, endFrameBound) => {
            ...
            if (node.renderTarget || node.screen) {
                const renderTarget = node.renderTarget || this.renin.screenRenderTarget;
                const width = ((boxHeight / 9) * 16) / this.zoomAmount;
                const height = boxHeight;
                const preview = new Mesh(makeRoundedRectangleBufferGeometry(1, 1, 4 / width, 4 / height, 32), new MeshBasicMaterial({ map: renderTarget.texture }));
        ...
        recurse(renin.root, 0, renin.root.startFrame, renin.root.endFrame === -1 ? renin.music.getDuration() * 60 : renin.root.endFrame);
    ...
    constructor(renin) {
        ...
        this.audioTrack = new Mesh(boxBufferGeometry, new MeshBasicMaterial({
        ...
        this.cuePoints = [
            new Mesh(boxBufferGeometry, new MeshBasicMaterial({
