import { WebGLRenderTarget, ShaderMaterial, Scene, OrthographicCamera, WebGLRenderer, CanvasTexture, Color, NoToneMapping } from 'three';
import { AudioBar } from './ui/AudioBar.mjs';
import { Sync } from './sync.mjs';
import defaultVert from './default.vert.mjs';
import { lerp } from './interpolations.mjs';
import { colors } from './ui/colors.mjs';
import { getWindowHeight, getWindowWidth } from './utils.mjs';
import { registerErrorOverlay } from './ui/error.mjs';
import { Music } from './music.mjs';
import { UIAnimation } from './ui/UIAnimation.mjs';
import { UIBox } from './ui/UIBox.mjs';
import screenShader from './ui/screenShader.mjs';
import performancePanelShader from './ui/performancePanel.mjs';
import { thirdsOverlayTexture } from './ui/thirdsOverlay.mjs';
import { performancePanelTexture } from './ui/performancePanelTexture.mjs';
import * as vite$1 from './ui/vite.mjs';
export { vite$1 as vite };
import * as ReninNode$1 from './ReninNode.mjs';
export { ReninNode$1 as ReninNode };

const defaultVertexShader = defaultVert;
...
class Renin {
    ...
    cuePoints = [];
    uiNeedsRender = true;
    renderTimesCPU = [...new Array(128)].map(() => 0);
    renderTimesCPUIndex = 0;
    renderTimesGPU = [...new Array(128)].map(() => 0);
    renderTimesGPUIndex = 0;
    renderer;
    ...
    root;
    screenRenderTarget = new WebGLRenderTarget(640, 360);
    isFullscreen = false;
    fullscreenAnimation = new UIAnimation();
    uiOldTime = Date.now() / 1000;
    uiTime = Date.now() / 1000;
    uiDt = 0;
    framePanelCanvas;
    framePanelTexture;
    query = null;
    updateTimes = [...new Array(128)].map(() => 0);
    updateTimesIndex = 0;
    uiUpdateTimes = [...new Array(128)].map(() => 0);
    uiUpdateTimesIndex = 0;
    memoryPercentages = [...new Array(128)].map(() => 0);
    memoryPercentagesIndex = 0;
    queryIsActive = false;
    options;
    constructor(options) {
        Renin.instance = this;
    ...
    uiUpdate() {
        ...
        this.screen.setSize(lerp(640, getWindowWidth(), this.fullscreenAnimation.value), lerp(360, (getWindowWidth() / 16) * 9, this.fullscreenAnimation.value));
        this.screen.object3d.position.x = lerp(getWindowWidth() / 2 - this.screen.object3d.scale.x / 2 - 16, getWindowWidth() / 2 - this.screen.object3d.scale.x / 2, this.fullscreenAnimation.value);
        this.screen.object3d.position.y = lerp(getWindowHeight() / 2 - this.screen.object3d.scale.y / 2 - 16, getWindowHeight() / 2 - this.screen.object3d.scale.y / 2, this.fullscreenAnimation.value);
...
export { Renin, defaultVertexShader };
